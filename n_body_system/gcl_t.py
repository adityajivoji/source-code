...
class Feature_learning_layer(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, input_c, hidden_c, output_c, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, norm_diff=False, tanh=False, input_reasoning=False,category_num=2):
        ...
        # MLP for node attention calculation
    ...
    def edge_model(self, h, coord, edge_attr=None):
        ...
        # Apply edge MLP to compute edge features
    ...
    def aggregate_coord(self, coord, edge_feat, coord_diff):
        # Compute factors using the edge features and the coordinate MLP
        # inconsistency: ap
        ...
        # Update the coordinates by adding the neighbor effect
    ...
    def aggregate_coord_reasoning(self, coord, edge_feat, coord_diff, category,h):
        ...
        # Apply factor MLP to compute updated factors
        factors = self.factor_mlp(factors)
        # Reshape factors tensor
    ...
    def node_model(self, x, edge_feat):
        # Extract dimensions from edge_feat tensor
        ...
        # Aggregate edge features by summing them while applying the mask
        aggregated_edge = torch.sum(mask*edge_feat,dim=2)
        # Concatenate the input features (x) with the aggregated edge features
        out = self.node_mlp(torch.cat([x,aggregated_edge],dim=-1))
        # If recurrent mode is enabled, add the input features to the output features
        if self.recurrent:
            out = x + out
        return out
    ...
    def non_linear(self, coord):
        ...
        # Center the coordinates by subtracting the mean coordinate
        coord = coord - coord_mean
        # Apply linear transformations to the coordinates using layer_q and layer_k
    ...
    def forward(self, h, coord, vel, edge_attr=None, node_attr=None,category=None): 
        ...
        if self.input_reasoning:
            ...
            # Perform regular coordinate aggregation
            coord = self.aggregate_coord(coord, edge_feat, coord_diff)
        # Add the coordinate velocity to the coordinates
        coord += self.coord_vel(vel.transpose(2,3)).transpose(2,3)

        if self.add_non_linear:
            # Apply non-linear transformation to the coordinates
            coord = self.non_linear(coord) # final output of Equivariant Geometric Feature Learning
...
def hat(v: torch.Tensor):
    ...
    # vectors are aligned along the second dimension
    x, y, z = v[:, 0], v[:, 1], v[:, 2]
