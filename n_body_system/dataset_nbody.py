...
class NBodyDataset():
    def __init__(self, partition='train', max_samples=1e8, dataset_name="nbody", past_length=20, future_length=20, training=True):
        # Initialize the NBodyDataset class with specified parameters
        self.partition = partition
        # Determine the suffix based on the partition
        ...
        # Append a suffix based on the dataset name
        ...
        self.dataset_name = dataset_name
        # Load the data and edges
        # data contains loc, vel, edge_attr, charges
    ...
    def load(self):
        # Load the trajectory data from files
        ...
        # Transpose the loc and vel arrays
        ...
        vel[:,:,1:] = loc[:,:,1:] - loc[:,:,:-1]
        vel[:,:,0] = vel[:,:,1]
        # Return the preprocessed data and edges
    ...
    def preprocess(self, loc, vel, edges, charges):
        ...
        n_nodes = loc.size(2)
        # Limit the number of samples for loc and vel
        loc = loc[0:self.max_samples, :, :, :]  # limit number of samples
        vel = vel[0:self.max_samples, :, :, :]  # speed when starting the trajectory
        # Select the charges for the limited number of samples
        charges = charges[0:self.max_samples]
        # Convert edges to a torch tensor
        edge_attr = torch.Tensor(edges)

        #Initialize edges and edge_attributes
        # assuming that everyine is connected to everyone
        rows, cols = [], []
    ...
    def __getitem__(self, i):
        # Retrieve the data for loc, vel, edge_attr, and charges
        loc, vel, edge_attr, charges = self.data
        # Select the data for the given index i
        loc, vel, edge_attr, charges =  loc[i], vel[i], edge_attr[i], charges[i]
        # Define the starting and ending frames for the trajectory
        ...
        if self.training:
            ...
            # Return data for validation/testing
            return loc[:,0:frame_0], vel[:,0:frame_0], edge_attr, charges, loc[:,frame_0:frame_T]
