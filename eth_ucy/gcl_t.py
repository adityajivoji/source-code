...
class Feature_learning_layer(nn.Module):
    ...
    def aggregate_coord_reasoning(self, coord, edge_feat, coord_diff, category,h,valid_mask):
        # Extract dimensions from coordinate tensor
        batch_size, agent_num, channels = coord.shape[0], coord.shape[1], coord.shape[2]
        # Repeat and reshape tensors for element-wise operations
        h1 = h[:,:,None,:].repeat(1,1,agent_num,1)
    ...
    def node_model(self, x, edge_feat,valid_mask):
    ...
    def inner_agent_attention(self,coord,h,valid_mask_agent,num_valid):
        agent_num = h.shape[1]
        ...
        v = coord - torch.mean(coord*valid_mask_agent,dim=(1,2),keepdim=True)* (agent_num/num_valid[:,None,None,None])
    ...
    def non_linear(self, coord, valid_mask_agent,num_valid):
        agent_num = coord.shape[1]
        coord_mean = torch.mean(coord*valid_mask_agent,dim=(1,2),keepdim=True)* (agent_num/num_valid[:,None,None,None])
    ...
    def forward(self, h, coord, vel, valid_mask, valid_mask_agent, num_valid, edge_attr=None, node_attr=None,category=None): 
