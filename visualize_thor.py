...
import argparse
import torch
from thor.processed.dataloader import Thor
from thor.model_t import EqMotion
import os
import numpy as np
import matplotlib.pyplot as plt
# from tqdm import tqdm
...
def plot_trajectory(_loc, _loc_end, loc_pred_head, epoch=0, save_dir=None, save_name= 'thor_trajectory.png'):
    prediction_head = 1
    num_agent = _loc.shape[0]
    for index in range(prediction_head):
        plt.figure()  # Create a new figure for each plot
        for i in range(num_agent):
            loc = _loc[i]
            loc_end = _loc_end[i]
            # loc_pred = loc_pred_head[index][i]
    ...
    with torch.no_grad():
        for _, data in enumerate(loader_test):
            if data is not None:
                ...
                loc_pred = loc_pred.squeeze()[None,:,:,:]
                index = np.random.randint(0,loc.shape[0])
                print(loc.shape, loc_end.shape, loc_pred.shape)
                plot_trajectory(loc, loc_end,loc_pred, epoch, save_dir=save_dir)
                break
...
if  __name__ == "__main__":
    ...
    parser.add_argument('--tanh', type=eval, default=False, metavar='N',
                        help='use tanh')
    parser.add_argument('--subset', type=str, default='Exp_2_run_5',
                        help='Name of the subset.')
    parser.add_argument('--model_save_dir', type=str, default='thor/saved_models',
                        help='Name of the subset.')
    parser.add_argument("--apply_decay",action='store_true')
    parser.add_argument("--supervise_all",action='store_true')
    parser.add_argument('--model_name', type=str, default='thor_best', metavar='N',
                        help='dataset scale')
    ...
    args.channels = 512
    args.tanh = True
    raw_files = os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)),'thor', 'raw_dataset'))
    raw_files = [file for file in raw_files if file.endswith('.tsv')]
    i = 1
    for file in raw_files:
        print("File: ", file)
        dataset_test = Thor(file[:-4], args.past_length, args.future_length, phase="train")

        loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=1, shuffle=False, drop_last=False,
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(loader_test):
                if data is not None:
                    ...
                    index = 5
                    loc_pred = loc_end
                    # print(loc_pred.shape, loc.shape, loc_end.shape)
                    plot_trajectory(loc, loc_end,loc_pred[None,:,:,:], epoch=i, save_dir='thor/img/', save_name= file[:-4] + '.png')
                    i += 1
                    break
