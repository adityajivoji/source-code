...
class EqMotion(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, in_channel, hid_channel, out_channel, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, recurrent=False, norm_diff=False, tanh=False, dct=True):
        ...
        self.predict_head = []
        for i in range(self.num_pred_head):
    ...
    def forward(self, h, x, vel, num_valid, edge_attr=None):
        ...
        x = x.view(batch_size,agent_num,self.num_pred_head,self.out_channel,-1)
